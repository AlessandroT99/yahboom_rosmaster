...
echo "Waiting for simulation to initialize (checking /clock topic)..."
# Wait until /clock appears (simulation publishing simulated time) or timeout
timeout=60
elapsed=0
interval=1
while [ $elapsed -lt $timeout ]; do
    if ros2 topic list 2>/dev/null | grep -q "^/clock$" &&
       ros2 topic list 2>/dev/null | grep -q "^/joint_states$"; then
        echo "Simulation and robot state are ready."
        break
    fi
    sleep $interval
...
if [ $elapsed -ge $timeout ]; then
    echo "Timed out waiting for simulation to initialize. Proceeding anyway."
fi
